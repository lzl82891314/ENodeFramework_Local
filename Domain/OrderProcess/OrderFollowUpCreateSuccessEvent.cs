...
{
    public class OrderFollowUpCreateSuccessEvent : DomainEvent<string>
    {
        ...
        private OrderFollowUpCreateSuccessEvent() { }

        public OrderFollowUpCreateSuccessEvent(ProcessingOrder processModel, string orderId, long ownerSoufunId, int followStageId, string statusName, string followUpDesc, DateTime createTime, int pageSource = 0, int orderMode = 0, long opId = 0, string opUerName = "", int identityType = -1)
            : base(processModel)
        {
            OrderId = orderId;
